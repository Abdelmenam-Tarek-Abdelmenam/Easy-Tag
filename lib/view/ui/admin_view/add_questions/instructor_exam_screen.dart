...
class InstructorExamScreen extends StatefulWidget {
  InstructorExamScreen({this.quiz,Key? key}) : super(key: key);
  Quiz? quiz ;
  @override
...
class _InstructorExamScreenState extends State<InstructorExamScreen> {
  ...
  late TextEditingController quizController = TextEditingController(text: quiz.title);
  late TextEditingController quizDescriptionController = TextEditingController(text: quiz.description);
  late TextEditingController quizTimeController = TextEditingController(text: quiz.timeout.inMinutes.toString());

  late List<TextEditingController> questionController = List.generate(quiz.questions.length,
          (index) => TextEditingController(text: quiz.questions[index].text));
  late List<TextEditingController> questionHintController = List.generate(quiz.questions.length,
          (index) => TextEditingController(text: quiz.questions[index].hint));

  late List<List<TextEditingController>> answerControllers = List.generate(quiz.questions.length,
          (index) => List.generate(quiz.questions[index].answers.length,
                  (i)=> TextEditingController(text: quiz.questions[index].answers[i])));

  late Quiz quiz = widget.quiz ?? Quiz(
      questions: [],
      timeOutMinutes: 10,
      title: '',
          ...
          children: [
            Padding(
              ...
              child: Row(
                ...
                children:  [
                  Text(currentPage == 0 ? 'Title and Duration' :'Question $currentPage/${quiz.questions.length}',
                    style: const TextStyle(fontSize: 28,fontWeight: FontWeight.w200),),
                  Row(
                    children: [
                      IconButton(onPressed: removeQuestion, icon: const Icon(Icons.clear,size: 30,color: Colors.red,)),
                      IconButton(onPressed: addQuestion, icon: const Icon(Icons.add_circle_sharp,size: 30,color: Colors.green,)),
                    ],
                  )
                ],
              ),
                ...
                child: Stack(
                  children: [
                    ...
                    children: [
                            ...
                            child: Column(
                              children: [
                                ...
                                TextField(
                                  ...
                                  onChanged: (text){ quiz.title = text;},
                                    ...
                                    Expanded(
                                      child: Padding(
                                        ...
                                        child: TextField(
                                          ...
                                          onChanged: (text){ quiz.setTimeout = int.parse(text);},
                                    ...
                                    TextButton(
                                      ...
                                      style: ButtonStyle(
                                        ...
                                        backgroundColor: MaterialStateProperty.all(ColorManager.blackColor),
                                          fixedSize: MaterialStateProperty.all(const Size.square(70))
                                      ),
                                    ),
                                    const SizedBox(width: 10,),
                                    ElevatedButton(onPressed: increaseDuration,
                                ...
                                TextField(
                                  ...
                                  onChanged: (text){ quiz.description = text;},
                      ...
                      ...quiz.questions.asMap().map((i,question) => MapEntry(i,
                          Container(
                              ...
                              child: Column(
                                children: [
                                  TextField(
                                    ...
                                    onChanged: (text){ quiz.questions[i].text = text;},
                                    keyboardType: TextInputType.multiline,
                                    minLines: 1,
                                    maxLines: 8,
                                    decoration: const InputDecoration(
                                  ...
                                  const SizedBox(height: 10,),
                                  TextField(
                                    controller: questionHintController[i],
                                    onChanged: (text){ quiz.questions[i].hint = text;},
                                    keyboardType: TextInputType.multiline,
                                    minLines: 1,
                                    maxLines: 3,
                                    decoration: const InputDecoration(
                                  ...
                                  quiz.questions[i].img == null ? TextButton(
                                    style: ButtonStyle(
                                        backgroundColor: MaterialStateProperty.all(ColorManager.blackColor),
                                  ...
                                  ListView.separated(
                                    ...
                                    primary: false,
                                    shrinkWrap: true,
                                    itemCount: answerControllers[i].length,
                                    itemBuilder: (ctx, index) {
                                      return Row(
                                        children: [
                                          Expanded(
                                            child: TextField(
                                              controller: answerControllers[i][index],
                                              onChanged: (text){ quiz.questions[i].answers[index] = text;},
                                          ...
                                          Transform.scale(
                                            ...
                                            child: Checkbox(
                                              ...
                                              value: quiz.questions[i].rightAnswer[index] == 1,
                                              onChanged: (value) {
                                                setState(() {
                                                  quiz.questions[i].rightAnswer[index] = (value??false)? 1:0;
                                                });
                                              },
                                            ),
                                          ),
  ...
  void saveQuiz() {
    ...
    print("timeout : ${quiz.timeout}");
    print("id : ${quiz.id}");
    print("description : ${quiz.description}");
    quiz.questions.forEach((element) {
      print("question text : ${element.text}");
    });

    // use quiz parameter
    // the Quiz is ready to upload
  }
  ...
  void pickImageAndUpload() async{
    ...
    if (photo != null) {
      ...
      String link = await uploadFile(File(photo.path));
  ...
  Future<String> uploadFile(File file) async {
    ...
    Reference ref = storage.ref().child("Logos").child("file" + DateTime.now().toString());
  ...
  void removeQuestion() {
    setState(() {
      ...
      if(currentPage > quiz.questions.length){currentPage = quiz.questions.length;}
      refreshControllers();
    });
  }
  ...
  void refreshControllers(){
    ...
    quizTimeController = TextEditingController(text: quiz.timeout.inMinutes.toString());
    ...
    answerControllers = List.generate(quiz.questions.length,
            (index) => List.generate(quiz.questions[index].answers.length,
                (i)=> TextEditingController(text: quiz.questions[index].answers[i])));
  }
